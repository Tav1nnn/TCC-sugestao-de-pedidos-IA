...
const RestEdit2 = () => {
    ...
    const handleCreateIngredient = async () => {
        const token = localStorage.getItem('authToken');
        try {
            await axios.post('http://localhost:8080/api/ingredients', { name: ingredient }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    const RemoveIngredientToDish = async (dishId, ingredientIdToRemove) => {
        const confirmed = window.confirm("Quer mesmo remover esse ingrediente?");
        if (!confirmed) return;
        const token = localStorage.getItem('authToken');
                                                        ...
                                                        <Dialog.Body>
                                                            ...
                                                            <NativeSelect.Root mt={4} >
                                                                <NativeSelect.Field
                                                                    ...
                                                                    borderBottom={'1px solid #A10808'}
                                                                >
                                                                    {editedMenuData.map((cat) => (
                                                                        <option key={cat.id} value={cat.categoryId} style={{padding: '2px', color: '#fff', backgroundColor: '#2D2C31' }}>
                                                                            {cat.category} {/*Alterar para cat.name quando mudar o editedMenuData para AllCategories, quebra a criação porque manda o nome ao invés do id*/}
                                                                        </option>
                                                                    ))}
                                                                </NativeSelect.Field>
                                                            </NativeSelect.Root>
                                                ...
                                                <Dialog.Positioner>
                                                    <Dialog.Content m={20} minH={200} maxH={'auto'} backgroundColor={"#2D2C31"} className="DialogContent" p={4} borderRadius="md" borderWidth="1px" borderColor="#A10808">
                                                        ...
                                                        <NativeSelect.Root variant={'subtle'}>
                                                            ...
                                                            >
                                                                {editedMenuData.map((cat) => (
                                                                    <option key={cat.categoryId} value={cat.category} style={{padding: '2px', color: '#fff', backgroundColor: '#2D2C31' }}>
                                                                        {cat.category}
                                                                    </option>
                                                                ))}
                                                            </NativeSelect.Field>
                                                        ...
                                                        <Dialog.Footer className="DialogFooter" mt={4}>
                                                            <Button variant="outline" size="sm" onClick={handleLocalSave} border={'1px solid #A10808'} color={'white'}>
                                                                <FaCheck />
                                                            </Button>
                                                            <Dialog.ActionTrigger asChild>
